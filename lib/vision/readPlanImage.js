// ============================================================
// VERIBUILD VISION - READ PLAN (PDF NATIVE)
// Sends a PDF directly to Claude Sonnet 4.5 using the
// document content block. Claude reads the PDF natively —
// no rasterization, no image conversion.
//
// Uses @anthropic-ai/sdk. Loaded dynamically so the module
// can be imported without the dependency until it is called.
// ============================================================
...
// Anthropic's documented cap for a base64-encoded PDF is 32 MB.
// Requests larger than this must be rejected before sending.
const MAX_PDF_BYTES = 32 * 1024 * 1024;

/**
 * Send a PDF directly to Claude and return structured measurements.
 *
 ...
 * }>}
 */
export async function readPlanImage(pdfBuffer) {
  const result = {
    success: false,
  ...
  try {
    if (!pdfBuffer || !Buffer.isBuffer(pdfBuffer)) {
      result.error = 'readPlanImage requires a PDF Buffer';
      return result;
    }

    if (pdfBuffer.length > MAX_PDF_BYTES) {
      result.error = `PDF is ${(pdfBuffer.length / 1024 / 1024).toFixed(1)} MB, exceeds 32 MB limit`;
      return result;
    }

    // Pre-flight cost estimate: PDF pages cost approximately
    // 1500–3000 tokens each depending on text density.
    // Assume worst case 3000 tokens per page, capped at 30 pages.
    ...
    if (preflightCost > MAX_COST_PER_PLAN_USD) {
      // This branch is unlikely to fire but guards against future
      // changes in pricing or limits.
    ...
    if (extractedCount === 0) {
      result.error = 'No usable measurements extracted from the PDF';
