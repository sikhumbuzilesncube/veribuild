...
// The prompt sent to Claude Sonnet 4.5 alongside the plan image.
...
// Insert the plan image alongside this prompt.
// ------------------------------------------------------------
export const USER_PROMPT = `Examine the attached floor plan image and return a JSON object with the following schema.
...
Guidance:
- "rooms" labels should be the actual names printed on the plan. Examples: Lounge, Kitchen, Bedroom 1, Bathroom, Garage, Dining, Study, Passage, Store.
...
// ------------------------------------------------------------
export const RETRY_PROMPT = `The previous response was not valid JSON. Return only the JSON object matching the schema. No prose. No markdown. No code fences. If a field cannot be determined, set it to null with confidence 0.`;

// ------------------------------------------------------------
// Test: this prompt strategy was chosen to match the fields
// already used by app/actions/readPlan.js and app/dashboard/verify/[id]/page.js
// so that the vision layer can write into the same project columns
