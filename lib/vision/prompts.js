...
- Wall length is the second most important field. If the plan shows external dimensions, sum them. If only some are readable, sum what is legible and note the coverage.`;
...
CRITICAL GUIDANCE FOR PROJECT NAME:

The project name appears in the title block or drawing notes, usually at the bottom-right corner or along the top of the drawing sheet.

Common formats to look for:
- "PROPOSED OUTBUILDING ON STAND 2774 TSHABALALA"
- "NEW RESIDENCE FOR MR MOYO"
- "FLOOR PLAN FOR 3-BEDROOM HOUSE"
- "PROPOSED COTTAGE - LOT 12 BORROWDALE"
- "SUBMISSION DRAWINGS FOR CHIKADAYA"

Extract a short, sensible project name, up to 60 characters.
If the plan contains multiple names (owner, architect, stand number), prefer the one that describes the building or the project.

If no clear project name is visible, set the value to null.
...
1. If the plan title block or a dimension table shows a total floor area, use that value.
...
4. If only some rooms are dimensioned, sum what is dimensioned and note the partial coverage in your "notes" field.
...
CRITICAL GUIDANCE FOR WALL LENGTH:

Wall length is the second most important field after floor area.
Extract it using the following priority order:

1. If the plan shows overall external dimensions along the perimeter, sum them.
2. If the plan shows individual wall segments with dimensions, sum the external segments.
3. If only some segments are dimensioned, sum what is legible and note the coverage.
4. If no wall dimensions are readable at all, set the value to null.

Do not confuse internal partition walls with external perimeter walls.
The wall_length should reflect only the external perimeter.
...
If the plan is a photograph (as opposed to a scan or digital export):
...
- If the photo has glare or shadows obscuring a label, omit that label
  rather than guess.
- If the entire image is very low contrast, blurred, or partly out of frame,
...
- Floor area and wall length must be extracted if any dimensions are legible.
- Project name should be the short name from the title block.
