...
//
// RURAL CODES are prefixed with R (C.R1, C.R2, etc.) so that
// the rate lookup cannot confuse them with residential codes.
// ============================================================
...
export function calcRoof(inputs, prices, isRural = false) {
  ...
  if (isRural) {
    ...
    // Codes prefixed with C.R to avoid rate collision.
    // ----------------------------------------------------------

    // Purlins
    const purlinSpacing = 1.2;
    const slopeLength = Math.sqrt(
      Math.pow(floorArea / wallLength, 2) + Math.pow(wallLength / 4, 2)
    );
    ...
    if (roofType === 'IBR' || roofType === 'Chromadek') {
      ...
      if (roofType === 'asbestos') {
        const sheetCount = ceilNum(roofArea / 1.2);
        items.push({
          code: 'C.R2',
