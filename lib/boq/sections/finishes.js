...
export function calcFinishes(inputs, prices) {
  ...
  // ----------------------------------------------------------
  // D.2 External wall plaster
  // ----------------------------------------------------------
  const externalWallArea = wallLength * wallHeight * 0.9;
  ...
  items.push({
    ...
    section: 'D',
    source: 'formula',
    formula: 'wallArea * 0.0231 m3/m2 * 1.10',
    parentCode: 'D.2',
  });

  // ----------------------------------------------------------
  // D.3 Rhinoset skim coat
  // ----------------------------------------------------------
  items.push({
    code: 'D.3',
  ...
  const screedCement = screedArea * 0.42;
  ...
  // ----------------------------------------------------------
  // D.5 Dry area floor tiles
  // ----------------------------------------------------------
  const dryFloorArea = floorArea * (1 - wetAreaMultiplier);

  items.push({
    code: 'D.5',
    description: 'Ceramic floor tiles 400 x 400mm, laid on 20mm cement and sand bed, in dry areas',
    unit: 'm2',
    qty: round1(dryFloorArea * 1.10),
    category: 'Finishes',
    section: 'D',
    source: 'formula',
  ...
  // ----------------------------------------------------------
  // D.7 Wet area wall tiles
  // ----------------------------------------------------------
  const wetWallTileArea = wetFloorArea * 2.0;
  ...
  items.push({
    ...
    unit: 'bag',
    qty: ceilNum(withWaste(wetWallTileArea * TILING.adhesiveKgPerM2 / 20, 0.05)),
    category: 'Finishes',
    section: 'D',
    source: 'formula',
    formula: 'wallTileArea * 0.167 kg/m2 / 20kg * 1.05',
    parentCode: 'D.7',
  });
  ...
  items.push({
    ...
    section: 'D',
    source: 'formula',
    formula: 'wallTileArea * 0.1 kg/m2 / 5kg * 1.05',
    parentCode: 'D.7',
  });

  // ----------------------------------------------------------
  // D.8 Internal wall paint
  // ----------------------------------------------------------
  const internalPaintLitres = Math.ceil(
    (netInternalWallArea / PAINT_COVERAGE.pvaPlaster_1st) * 5 +
    (netInternalWallArea / PAINT_COVERAGE.pvaPlaster_subsequent) * 5 * 2
  );
  ...
  items.push({
    ...
    section: 'D',
    source: 'formula',
    formula: 'floorArea / 2.88 m2 per sheet * 1.05',
  });
  ...
  items.push({
    ...
    unit: 'kg',
    qty: round1((ceilingArea * CEILING.cloutNailsPerM2) / 250),
    category: 'Finishes',
    section: 'D',
    source: 'formula',
  ...
  // D.12 Skirting
  // ----------------------------------------------------------
  const skirtingLength = wallLength * 0.9;
...
// ------------------------------------------------------------
// Wet area detection.
// Recognises Zimbabwe and UK room labels for rooms that
// typically receive wall tiles and slip-resistant floor tiles.
// Returns the proportion of floor area considered wet.
// Capped at 35% to avoid over-allocation on unusual plans.
// ------------------------------------------------------------
function detectWetAreas(roomLabels) {
  if (!roomLabels) return 0.15;
  ...
  let wetCount = 0;
  for (const kw of wetKeywords) {
    const escaped = kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const re = new RegExp(`\\b${escaped}`, 'g');
    const matches = labels.match(re);
    if (matches) wetCount += matches.length;
  }

  if (wetCount === 0) return 0.15;

  return Math.min(wetCount * 0.08, 0.35);
}
