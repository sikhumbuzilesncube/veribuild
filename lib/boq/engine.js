...
export function generateFullBOQ(project, options = {}) {
  ...
  // Attach rates and amounts. Headers get rate 0; their rolled-up amount is
  // computed at display time (page-level rollUpHeaderAmounts helper).
  ...
  // Section F: labour. Each row is now a trade summary with both gang-days
  // (crew time) and man-days (total effort) reported.
  ...
  // Backward-compatible flat list. isHeader and parentCode preserved.
  const flatItems = [];
...
// ------------------------------------------------------------
// Rate lookup: map BOQ item code to price key.
// Returns 0 for header items (their cost lives on sub-lines).
// ------------------------------------------------------------
