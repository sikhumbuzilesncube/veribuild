...
export default function AdminConstructionPage() {
  ...
  const filteredCompanies = filter === 'all' 
    ? companies 
    : companies.filter(c => c.subscription_status === filter);
  ...
  const stats = {
    ...
    pending: companies.filter(c => c.is_verified === false).length,
  };
      ...
      <div className="flex">
        ...
        <div className="fixed left-0 top-0 h-full w-64 bg-[#2C3E50] text-white p-6">
          ...
          <nav className="space-y-2">
            ...
            <Link href="/admin/construction" className="block py-2 px-4 bg-[#F47B20] rounded-lg font-medium">
              🏗️ Construction
            </Link>
        ...
        <div className="ml-64 p-6 w-full">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold text-[#2C3E50]">🏗️ Construction Companies</h1>
              <p className="text-gray-600">Manage all construction companies</p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            {filteredCompanies.length > 0 ? (
                  ...
                  <tbody>
                    {filteredCompanies.map((company) => (
                      <tr key={company.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                        <td className="px-4 py-3 font-medium text-[#2C3E50]">{company.company_name}</td>
                        <td className="px-4 py-3 text-gray-600">{company.contact_person}</td>
                        <td className="px-4 py-3 text-gray-600">{company.location || 'Not set'}</td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            company.subscription_status === 'active' ? 'bg-green-100 text-green-700' :
                            company.subscription_status === 'inactive' ? 'bg-yellow-100 text-yellow-700' :
                        ...
                        <td className="px-4 py-3">
                          <Link
                            href={`/admin/construction/${company.id}`}
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">No construction companies found</p>
              </div>
            )}
          </div>
        </div>
